...
const Card = ({ icon: Icon, title, description, badge, isRed = false }) => (
  <motion.div
    whileHover={{ y: -5 }}
    className={`relative p-8 rounded-[3rem] border transition-all duration-300 group ${isRed
  ...
  >
    <div className={`mb-8 w-14 h-14 rounded-2xl flex items-center justify-center ${isRed ? 'bg-white/20' : 'bg-ncsu-red/10 text-ncsu-red'
      }`}>
      <Icon size={24} />
    </div>
    <span className={`text-[10px] font-black uppercase tracking-[0.2em] mb-4 block ${isRed ? 'text-white/70' : 'text-ncsu-red'
      }`}>
      {badge}
    </span>
    ...
    <section id="problem" className="relative z-10 py-12 bg-white overflow-hidden">
      <div className="container mx-auto px-6">
        ...
        <div className="mt-20 max-w-5xl mx-auto bg-slate-50 p-1.5 rounded-[3rem] shadow-xl shadow-slate-100/50 border border-slate-100 flex flex-col md:flex-row items-center gap-12 px-12 py-12">
          <div className="text-7xl font-black text-ncsu-red tracking-tighter italic">70%</div>
          <div className="text-lg font-medium text-slate-500 border-l border-slate-200 pl-12 leading-relaxed">
            of teachers lack adequate funding to provide <span className="font-black text-ncsu-black italic underline decoration-ncsu-red">high-quality</span> science instruction for their students, according to PBS.
          </div>
        </div>
      </div>
    </section>
